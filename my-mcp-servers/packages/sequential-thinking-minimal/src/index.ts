...
class SequentialThinkingAPI {
  private steps: Array<{ id: string; content: string; timestamp: Date }> = [];

  async createStep(content: string, id?: string) {
  ...
  async generateSolution(context: string, requirements: string[]) {
    const solution = {
      context,
      requirements,
